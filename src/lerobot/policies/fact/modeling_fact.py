...
class FACTPolicy(PreTrainedPolicy):
    """
    Action Chunking Transformer Policy as per Learning Fine-Grained Bimanual Manipulation with Low-Cost
    Hardware (paper: https://huggingface.co/papers/2304.13705, code: https://github.com/tonyzhaozh/fact)
    """

    config_class = FACTConfig
    name = "fact"
    ...
    def forward(self, batch: dict[str, Tensor]) -> tuple[Tensor, dict]:
        ...
        batch_action = batch[ACTION][..., selected_action_indices] # B, S, 8
        batch_6d_rot = batch["action_rotation"]
        batch_action = torch.cat([batch_action, batch_6d_rot], dim=-1) # B, S, 20
        
        l1_loss = (
            F.l1_loss(batch_action, actions_hat, reduction="none") * ~batch["action_is_pad"].unsqueeze(-1)
        ).mean()
...
class FACTTemporalEnsembler:
    def __init__(self, temporal_ensemble_coeff: float, chunk_size: int) -> None:
        ...
        NOTE: The default value for `temporal_ensemble_coeff` used by the original FACT work is 0.01. This
...
class  FACT(nn.Module):
    ...
    def __init__(self, config: FACTConfig):
        ...
        if self.config.use_vae:
            self.vae_encoder = FACTEncoder(config, is_vae_encoder=True)
        ...
        self.encoder = FACTEncoder(config)
        self.decoder = FACTDecoder(config)
        ...
        self.action_head = nn.Linear(config.dim_model, self.config.selected_action_shape[0])
    ...
    def forward(self, batch: dict[str, Tensor]) -> tuple[Tensor, tuple[Tensor, Tensor] | tuple[None, None]]:
        ...
        if self.config.use_vae and ACTION in batch and self.training:
            ...
            pos_embed = self.vae_encoder_pos_enc.clone().detach()  # (1, S+4, D)
...
class FACTEncoder(nn.Module):
    ...
    def __init__(self, config: FACTConfig, is_vae_encoder: bool = False):
        super().__init__()
    ...
    def forward(
        self, x: Tensor, cond: Tensor| None=None, pos_embed: Tensor | None = None, key_padding_mask: Tensor | None = None
    ) -> Tensor:
        for layer in self.layers:
            x = layer(x,cond=cond, pos_embed=pos_embed, key_padding_mask=key_padding_mask)
        x = self.norm(x)
        return x
...
class ACTEncoderLayer(nn.Module):
    def __init__(self, config: FACTConfig):
        ...
        self.mlp_mod = _AdaLNMod(config.dim_model)
        self.mlp_gate = _ZeroGate(config.dim_model)
        self.mlp_mod.reset_parameters()
    ...
    def forward(self, x, cond: Tensor| None=None, pos_embed: Tensor | None = None, key_padding_mask: Tensor | None = None) -> Tensor:
        skip = x
        if self.pre_norm:
            x = self.norm1(x)
        q = k = x if pos_embed is None else x + pos_embed
        x, weights = self.self_attn(q, k, value=x, key_padding_mask=key_padding_mask, average_attn_weights=True)
        if not self.training:
        ...
        if cond is not None:
            x = self.mlp_mod(x, cond)
        x = self.linear2(self.dropout(self.activation(self.linear1(x))))
        if cond is not None:
            x = self.mlp_gate(self.dropout2(x), cond)
        else:
            x = self.dropout2(x)
        x = skip + x
...
class FACTSinusoidalPositionEmbedding2d(nn.Module):
